...
var $PRZ;   
  function csv_query($blz) {
    $cdata = -1;
    $fp = fopen(FOLDER_RELATIV_INCLUDES . 'data/blz.csv', 'r');
  ...
  function db_query($blz) {
    $blz_query = go_db_query("SELECT * from banktransfer_blz WHERE blz = '" . $blz . "'");
    if (go_db_num_rows($blz_query)){
      $data = go_db_fetch_array ($blz_query);
    }else
      $data = -1;
    return $data;
  }  
  function query($blz) {
    if (CONSTITUENT_PAYMENT_BANKTRANSFER_DATABASE_BLZ == 'true' && defined(CONSTITUENT_PAYMENT_BANKTRANSFER_DATABASE_BLZ))
  ...
  function Mark30($AccountNo) {
    ...
    $Help = $this->Method06($AccountNo, '000765432', FALSE, 10, 11); 
      if ($Help != 0) {
        ...
        if ($Help != 0) {
          $Help = $this->Method06($AccountNo, '000065432',FALSE, 10, 9); 
        }
        if ($Help != 0) {
